...
df_i$Provincia.de.hospitalización <- str_replace_all(df_i$Provincia.de.hospitalización, "[0123456789]", "") %>% 
  ...
  str_replace_all("COMUNITAT VALENCIANA", "Comunidad Valenciana") %>% 
  str_replace_all("Coruña (A)", "A Coruña") %>% # Revisar que funcione correctamente
  ...
  str_replace_all("MADRID, COMUNIDAD DE", "Madrid") %>% # Revisar que funcione correctamente
  str_replace_all("Madrid*", "Madrid") %>%
  str_replace_all("MURCIA, REGIÓN DE", "Murcia") %>% # Revisar que funcione correctamente
  str_replace_all("Murcia*", "Murcia") %>% 
  str_replace_all("NAVARRA, COMUNIDAD FORAL DE", "Navarra") %>% # Revisar que funcione correctamente
  str_replace_all("Navarra*", "Navarra") %>% 
  str_replace_all("PAÍS VASCO", "País Vasco") %>% 
  str_replace_all("Palmas (Las)", "Las Palmas") %>% # Revisar que funcione correctamente
  str_replace_all("Palmas, Las", "Las Palmas") %>%
  str_replace_all("Rioja (La)", "La Rioja") %>% # Revisar que funcione correctamente
...
Verificación del tipo de la columna de provincias después de la limpieza
